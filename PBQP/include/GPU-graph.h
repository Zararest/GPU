...
struct Graph final {
  ...
  public:
    ...
    void addNodes(size_t DeviceNode, size_t HostNode) {
      assert(DeviceNodeToHostNode.find(DeviceNode) ==
             DeviceNodeToHostNode.end());
      if (!MaxDeviceIdx)
    ...
    std::optional<size_t> getMaxDeviceIdx() const { return MaxDeviceIdx; }
  };
...
private:
  ...
  __host__ void updateTranslator();
  __host__ void updateHostAdjMatrix();
  __host__ void removeUnreachableCosts();
...
public:
  __host__ Graph() = default;

  __host__ Graph(const PBQP::Graph &HostGraph);

  __host__ void free();

  __host__ size_t getNumOfCostCombinations() const;

  __host__ size_t getNodeCostSize(unsigned NodeIdx) const {
  ...
  __host__ std::optional<size_t> getHostNode(size_t DeviceIdx) const {
  ...
  __device__ unsigned getNumOfCosts() const { return NumOfCosts; }
};
