...
def measure_performance(in_file, use_gpu):
    ...
    time_ms = int(result.stdout)  # Convert time to milliseconds
    return time_ms
...
def main():
  ...
  for nodes_num in range(first_measure, args.max_nodes + 1):
    if not CPU_too_slow:
      ...
      CPU_too_slow = time_cpu / time_gpu > 10
    else:
      time_gpu = measure_performance(out_files[nodes_num - first_measure], use_gpu=True)
      gpu_times.append(time_gpu)

  # Create the graph
  plt.plot(range(2, args.max_nodes + 1)[0:len(cpu_times)], cpu_times, label='CPU')
  plt.plot(range(2, args.max_nodes + 1)[0:len(gpu_times)], gpu_times, label='GPU')
