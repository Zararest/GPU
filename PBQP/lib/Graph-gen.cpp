...
host::Matrix<Graph::Cost_t> genMatrix(size_t H, size_t W, 
...
public:
  EdgesInfo(size_t NumOfNodes, size_t CurNodeIdx) :  NumOfNodes{NumOfNodes}, 
  ...
  void addNeighbors(double AvgNeighbNum, std::mt19937 &Gen) {
    ...
    auto NeighbNumDistrib = std::normal_distribution{AvgNeighbNum, AvgNeighbDiv};
    auto NeighbDistrib = std::uniform_int_distribution<size_t>{0ull, NumOfNodes - 1};
  ...
  size_t getCurNodeIdx() const {
    return CurNodeIdx;
  }

  auto neighbBegin() {
    return NewNeighbors.begin();
  }

  auto neighbEnd() {
    return NewNeighbors.end();
  }
};

void generateEdge(Graph &Clique, size_t LhsIdx, size_t RhsIdx, 
                  bool HasInfCosts, std::mt19937 &Gen) {
  auto &Lhs = *(Clique.nodesBeg() + LhsIdx);
  auto &Rhs = *(Clique.nodesBeg() + RhsIdx);
  auto Matrix = genMatrix(Lhs->costSize(), Rhs->costSize(), 
                          HasInfCosts, Gen);
  Clique.addEdge(*Lhs, std::move(Matrix), *Rhs);
}

template <typename It>
void addEdgesByInfo(Graph &Clique, It Beg, It End, 
                    bool HasInfCosts, std::mt19937 &Gen) {
  for (auto &Info : utils::makeRange(Beg, End)) {
    auto CurNodeIdx = Info.getCurNodeIdx();
    std::for_each(Info.neighbBegin(), Info.neighbEnd(), 
                  [&](size_t NeibIdx) {
                    generateEdge(Clique, CurNodeIdx, NeibIdx, 
                                 HasInfCosts, Gen);
                  });
  }
}

void addEdges(Graph &Clique, size_t VectSize, 
              double AvgNeighbNum, bool HasInfCosts, 
              std::mt19937 &Gen) {
  ...
  addEdgesByInfo(Clique, InfoForNodes.begin(), InfoForNodes.end(), 
                 HasInfCosts, Gen);
}

Graph generateClique(size_t NumOfNodes, size_t VectSize, 
                    double AvgNeighbNum, bool HasInfCosts, 
                    std::mt19937 &Gen, std::string CliquePrefix) {
  ...
  std::generate(CostVectors.begin(), CostVectors.end(), 
    [VectSize, &Gen]() { 
  ...
  auto Clique = Graph{};
  auto Idx = 0ull;
  std::for_each(CostVectors.begin(), CostVectors.end(), 
...
Graph generateGraph(GenConfig Cfg) {
  ...
  std::transform(CliqueSizes.begin(), CliqueSizes.end(),
                 CliqueIdxes.begin(),
                std::back_inserter(Cliques),
                [&Gen, &Cfg](size_t CliqueSize, size_t Idx) {
                  return generateClique(CliqueSize, Cfg.VectSize, 
                                        Cfg.AvgNeighbNum, Cfg.HasInfCosts,
                                        Gen, std::to_string(Idx));
                });
