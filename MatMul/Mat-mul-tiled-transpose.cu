...
__device__ void fillTilesTransposed(size_t Iteration, Tile &ATile, DevMatRowMajor A,
                          Tile &BTile, DevMatColMajor B) {
...
__global__ void transposedTileMatMulKernel(DevMatRowMajor A, DevMatColMajor B,
                                  DevMatRowMajor C) {
  ...
  for (size_t i = 0; i < NumOfTiles; ++i) {
    fillTilesTransposed(i, ATile, A, BTile, B);
    __syncthreads();
...
HostMatrix transposeTiledMatMul(const HostMatrix &A, const HostMatrix &B, bool PrintTime) {
  ...
  transposedTileMatMulKernel<<<BlockGridDim, ThrBlockDim>>>(DevA, DevB, DevC);
